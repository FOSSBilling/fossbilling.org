...
\*\*\*\*FOSSBilling introduces new functionality such as prevention against session hijacking, enforced session expropriations, proper usage of the HTTPS only flag for cookies, a HTTPS only mode, and more while also having resolved numerous security vulnerabilities which still effect BoxBilling.
