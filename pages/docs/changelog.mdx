...
### Breaking Changes
 ...
 - We've dropped official support for VestaCP due to it's unmaintained status, if you use a fork of VestaCP that was using it's server manager, you can find a backup of it [here](https://github.com/jaapmarcus/FOSSBilling-vesta/tree/main)
...
### Enhancements
 ...
 - The SpamChecker module had it's settings page visually reworked.
 ...
 - The Embed module had it's settings page visually reworked.
 - We've added in the `symfony/http-client` package and started to use that instead of curl `calls`, which improve compatibility.
 - The Pages and MassMailer modules have had their admin panel styling reworked.
 - The admin panel has had it's layout significantly improved. It's now much more responsive to different resolutions and the sidebar navigation has been replaced with a horizontal navbar.
