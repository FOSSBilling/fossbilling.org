...
## 0.4.0 (WIP)
Note: Note: FOSSBilling version 0.4.0 is not yet released. We believe this will be the final changelog, however if needed they may be further changes before we release version 0.4.0.
...
### New Features
 ...
 - The foss-update.php script has undergone visual enhancements and now includes support for performing file-level operations.
  - These file operations include deleting, renaming, and moving files which will allow us to make the update process smoother in the future and remove and any un-used dependencies.
