...
## 0.4.0 (WIP)
Note: FOSSBilling version 0.4.0 is still under development and as such, this changelog may not be fully representative of the final release.

### Breaking Changes
 - Both the client and admin `change_password` API endpoints to require password verification.
   - These are the required parameters now: `current_password`, `new_password`, and `confirm_password`.
 ...
 - We've dropped official support for VestaCP due to it's unmaintained status, if you use a fork of VestaCP that was using it's server manager, you can find a backup of it here
 - We've moved all twig filters that were outside of their class inside of it.

### Bug Fixes
 - Fixed the way email codes are generated.
 - Fixed the form builder module.
 ...
 - The Virtualmin server manager has had bugfixes Implemented.

## Enhancements
 - The CookieConsent module has had a visual rework.
 - The SpamChecker module had it's settings visually reworked.
 ...
 - Improved responsiveness to the admin panel.
 ...
 - The FormBuilder module has had it's styling rebuilt.

## New Features
 - We've refactored the way payment gateway logos are loaded, so they now no longer need to be hard-coded into a theme's CSS.
 - We've started implementing native JS modals for the Administrator panel, and have started using that instead of jQuery based solutions. 
