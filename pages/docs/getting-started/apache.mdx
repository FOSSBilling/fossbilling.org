...
#### Creating the MySQL database
    ...
    Creating a MySQL database is quite straight forward process.
    ...
    ```sudo mysql -u root -p```
    **Note** your username may be different if you have changed it you will need to use the username you have changed it to
    
    Let’s start by creating a MySQL database using the following command.
    in the given example our database name will be ```"fossbilling"```
    ```CREATE DATABASE fossbilling;```
    
    you now have a database named fossbilling that you can connect your panel to in later steps.
    
    you can now move on to the next section Creating the MySQL user
    
#### Creating the MySQL user
    
    we will create a MySQL user that we will assign to our new database. We can create this user by running the following command.
    we will be calling the user “example” and giving it the password “password“. When creating your own, make sure you replace both of these for security purpose DO NOT use "example" and "password".
    
    ```CREATE USER 'example'@'localhost' IDENTIFIED BY 'password';```
    
    Now with your user created and password assigned to it, we can now go ahead and grant all privileges to the user so that it can interact with the database.
    you can do that by simply using the following command m,ake sure to replace ```"example"``` with the username that you created above.
    
    ```GRANT ALL PRIVILEGES ON fossbilling.* TO 'example'@'localhost';```
    
    to finish off the installation of the MySQL user and database you will need to flush, this will refresh the user and database permissions if you do not do this step, you will not have access to the database under that user account.
    
    ```FLUSH PRIVILEGES;```
    
    you are not complete with the MySQL setp, make sure to note the information you just created Username, and Database you will need these moving on to the "Web Installer" section.
