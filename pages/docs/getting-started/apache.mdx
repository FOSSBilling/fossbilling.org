...
# <FontAwesomeIcon icon={faServer} /> Install on an Apache server
...
<Tabs items={['Ubuntu']}>
  <Tab>
    ...
    To enable `.htaccess` files to override the Apache configuration, we will need to edit the `apache2.conf` file. Open the file using your favorite text editor. We will use `nano` in this guide.
      ```bash filename="bash"
...
<Tabs items={['Ubuntu']}>
  <Tab>
    ...
    Now, use the builtin `mysql_secure_installation` script to secure our MySQL installation.
      ```bash filename="bash"
    ...
    Don't mistake this with the `root` account in your Ubuntu server, it's a different account and they are unrelated with each other. This is the `root` account for your MySQL server which will have full permissions over your MySQL server. Enter a strong password, press `Enter`, then confirm the password and press `Enter` again.
...
Once the installation is done, we will need to adjust the firewall settings to allow incoming traffic on port 80 (HTTP) and 443 (HTTPS).
