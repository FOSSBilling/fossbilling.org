...
5. Ensure you correctly configure your default currency. The price format **must** contain `{{price}}` in order for it to work correctly as this represents the price of your currency. Simply enter the currency either on the left or the right side of  `{{price}}`.
...
After completing the steps inside of the FOSSBilling installer, you're almost ready to log in and begin using the software!
Ensure you've followed these last few steps to validate that everything is working correctly.
...
We recommend configuring cron to run once ever 5 minutes. If 15 minutes goes past without cron executing, FOSSBilling will display a notification in the administrator panel. If you see this message, you need to resolve the issue as soon as possible.
...
If you ever need to find it again, navigate to `Settings` -> `Scheduled Tasks` (example.com/admin/extension/settings/cron).
...
To do this navigate to `Settings` -> `Scheduled Tasks` (example.com/admin/extension/settings/cron`) and then enable the cron API endpoint and click on "update"
