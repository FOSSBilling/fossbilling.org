...
6. Optional: Pull in the latest translations: `cd src/locale && git submodule update --init --remote --recursive`
	- Optionally, you may instead download the [latest release](https://github.com/FOSSBilling/locale/releases/tag/latest) from the locale repository and extract the contents to the `src/locale` directory. It's suggested to empty the contents of the directory first
