...
- **Recommended:** Use the automated migration tool.
  ...
  2. Login to the administrator panel, go to `System->update` in the top bar.
  3. Click the `Apply Patches & Update Configuration` button to run the automated config migrator.
    - The FOSSBilling interface should provide feedback if the migration went smoothly, but you can verify by opening `config.php`, you should see new configuration keys that were added. There should also now be a backup of the old config under `config.old.php`
