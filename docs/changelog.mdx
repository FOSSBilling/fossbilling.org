...
- FOSSBilling will log a stack trace when an exception is thrown with debug on. (`log_stacktrace` and `stacktrace_length` in the config file)
...
- Completely added rewritten plex integration
